...
func readFile(path string,offset []int)(int,string){
	f,err := os.OpenFile("/Users/macbook/Desktop/CSV/"+path,os.O_RDONLY,0644)
	if err != nil{
...
func (c *CSVX)Require(args *Requestx,reply *Resultx)error{
	...
	log.Println("count:", count, "data:",info)
	reply.Count=count
	reply.Result=info
	return nil
}
...
func main(){
	...
	rpc.Register(csvx)
	rpc.HandleHTTP()
	listener,e := net.Listen("tcp","0.0.0.0:5433")
	if e!=nil {
		log.Println(e)
	}
